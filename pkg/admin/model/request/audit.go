...
func (m AllQuery) Validate() error {
	return validation.ValidateStruct(
		&m,
		validation.Field(&m.TargetID,
			validation.Required.Error(errorcode.AuditInvalidTargetID),
			is.MongoID.Error(errorcode.AuditInvalidTargetID)),
		validation.Field(&m.Target,
			validation.In(constant.AffiliateAuditTarget...).Error(errorcode.AuditInvalidTarget)),
